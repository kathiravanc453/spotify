...
export default function Header({ search, setSearch, user, onLogout }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center justify-between px-4 md:px-8 py-3.5 bg-[#07070a]/60 backdrop-blur-xl border-b border-white/5">
      ...
      {/* Search bar */}
      <div className="relative flex-1 max-w-xs mx-3 md:mx-4 md:ml-auto md:max-w-xs">
        ...
        <input
          ...
          className="w-full bg-white/[0.04] hover:bg-white/[0.07] border border-white/5 focus:border-cyan-500/50 text-white placeholder-white/30 text-sm rounded-2xl pl-9 pr-10 py-2 focus:outline-none focus:ring-1 focus:ring-cyan-500/20 transition-all duration-300"
        />
        {search ? (
          <button
            onClick={() => setSearch('')}
